...
function hoot_meta_info_blocks( $args = '', $context = '', $editlink = true ) {
	...
	if ( !empty( $display['author'] ) ) :
		$blocks['author']['label'] = __( 'By:', 'dispatch-premium' );
		ob_start();
	...
	if ( !empty( $display['date'] ) ) :
		$blocks['date']['label'] = __( 'On:', 'dispatch-premium' );
	...
	if ( !empty( $display['cats'] ) ) :
		...
		if ( !empty( $category_list ) ) :
			$blocks['cats']['label'] = __( 'In:', 'dispatch-premium' );
			$blocks['cats']['content'] = $category_list;
		endif;
	endif;

	if ( !empty( $display['tags'] ) && get_the_tags() ) :
		$blocks['tags']['label'] = __( 'Tagged:', 'dispatch-premium' );
		$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'dispatch-premium' ) : get_the_tag_list( '', ', ', '' );
	endif;

	if ( !empty( $display['comments'] ) && comments_open() ) :
		$blocks['comments']['label'] = __( 'With:', 'dispatch-premium' );
		ob_start();
		comments_popup_link(__( '0 Comments', 'dispatch-premium' ),
							__( '1 Comment', 'dispatch-premium' ),
							__( '% Comments', 'dispatch-premium' ), 'comments-link', '' );
		$blocks['comments']['content'] = ob_get_clean();
	endif;

	if ( $editlink && $edit_link = get_edit_post_link() ) :
		$blocks['editlink']['label'] = '';
		$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'dispatch-premium' ) . '</a>';
	endif;
