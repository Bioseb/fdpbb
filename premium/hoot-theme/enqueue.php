...
function hoot_premium_base_enqueue_scripts() {
	...
	if ( !hoot_get_mod('disable_lightbox') &&
		...
		$script_uri = hoot_locate_script( 'js/lightGallery' );
	...
	if ( $archive_type == 'mosaic2' || $archive_type == 'mosaic3' || $archive_type == 'mosaic4' ) {
		$script_uri = hoot_locate_script( 'js/isotope.pkgd' );
...
function hoot_premium_extend_enqueue_scripts() {
	...
	wp_enqueue_script( 'hoot-theme-premium', $script_uri, array( 'hoot-theme' ), THEME_VERSION, true );
	/* Pass data to hoot scrollpoints */
	$padding = intval( hoot_get_mod( 'scrollpadding' ) );
	$data = ( empty( $padding ) ) ? array() : array( 'customScrollerPadding' => $padding );
	$data = apply_filters( 'hoot_localize_scrollpoints', $data );
	if ( !empty( $data ) )
		wp_localize_script( 'hoot-scrollpoints', 'hootData', $data );
}
