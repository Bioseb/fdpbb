	...
	if ( !empty( $font_families ) ) {
		...
		foreach ( $font_families as $key => $font )
			if ( isset( $fonts_list[ $font ] ) )
				$font_families[ $key ] = $fonts_list[ $font ] . ':' . apply_filters( 'hoot_google_fonts_styles', '300,300italic,400,400italic,700,700italic,800,800italic' );
			else
 ...
 */
remove_filter( 'hoot_fonts_list', 'hoot_theme_fonts_list' );