<div class="contact-info-widget">
	...
	<?php if ( !empty( $email ) ) :
		...
		<div class="contact-info-block">
			...
			<div class="contact-info-text"><a href="mailto:<?php echo antispambot( $email, 1 ); ?>" <?php hoot_attr( 'contact-info-link', 'email' ); ?>><?php echo $email; ?></a></div>
		</div>
	<?php endif; ?>
		...
		foreach( $profiles as $key => $profile ) : ?>
			<div class="contact-info-block">
				...
				elseif ( $profile['icon'] == 'fa-envelope' ) :
					echo '<div class="contact-info-icon"><i class="far fa-envelope-o"></i></div>';
					if ( !empty( $profile['url'] ) ) {
						$profile['text'] = ( !empty( $profile['text'] ) ) ? $profile['text'] : $profile['url'];
						echo '<div class="contact-info-text"><a href="mailto:' . antispambot( $profile['url'], 1 ) . '" ' . hoot_get_attr( 'contact-info-link', 'email' ) . '>' . esc_html( $profile['text'] ) . '</a></div>';
					};
				else: ?>
					<div class="contact-info-icon"><i class="<?php echo hoot_sanitize_fa( $profile['icon'] ); ?>"></i></div>
					<div class="contact-info-text"><?php
						if ( !empty( $profile['url'] ) )
							echo '<a href="' . esc_url( $profile['url'] ) . '" ' . hoot_get_attr( 'contact-info-link', 'profile' ) . '>';
