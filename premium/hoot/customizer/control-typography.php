	...
	public function render_content() {
		...
		switch ( $this->type ) {
			...
			case 'fontface' :
				...
				<input class="hoot-customizer-control-fontface" value="<?php echo esc_attr( $value ) ?>" <?php $this->link(); ?> type="hidden" data-label="<?php echo esc_attr( $selectedfont ); ?>"/>

				<div class="hoot-customizer-control-fontface-picked">
				...
				<div class="hoot-fontface-list">
					...
					foreach ( array(
						__( 'Websafe Fonts', 'dispatch-premium' ) => $webfonts,
						__( 'Google Web Fonts', 'dispatch-premium' ) => $googlefonts,
...
// Only load in customizer (not in frontend)
if ( class_exists( 'WP_Customize_Control' ) ) :
function hoot_customizer_fontface_control_interface ( $wp_customize, $id, $setting ) {
...
add_action( 'hoot_customizer_control_interface', 'hoot_customizer_fontface_control_interface', 10, 3 );
endif;
 ...
 * @return void
 */
function hoot_customizer_prepare_typography_settings( $value, $key, $setting, $count ) {
	...
	if ( $setting['type'] == 'typography' ) {
		...
		if( is_array( $setting['options'] ) && !empty( $setting['options'] ) ):
			...
			if( $size || $face || $style || $color ):
				...
				// Group Start
				$button = "<span class='hoot-typography-button' data-controlgroup='{$key}'>";
					if ( $face ) $button .= '<span class="hoot-typography-button-unit hoot-typography-button-face"></span>';
					if ( $size ) $button .= '<span class="hoot-typography-button-unit hoot-typography-button-size"></span>';
					if ( $style ) $button .= '<span class="hoot-typography-button-unit hoot-typography-button-style"></span>';
					if ( $color ) $button .= '<span class="hoot-typography-button-unit hoot-typography-button-color"></span>';
				$button .= '</span>';
				...
				if ( $color ) :
					...
					$value[ "{$key}-color" ] = array(
						'label'       =>  __( 'Font Color', 'dispatch-premium' ),
						'section'     => $setting['section'],
						'type'        => 'color',
						'priority'    => $setting['priority'],
				...
				if ( $size ) :
					...
					$value[ "{$key}-size" ] = array(
						'label'       =>  __( 'Font Size', 'dispatch-premium' ),
						'section'     => $setting['section'],
						'type'        => 'select',
						'priority'    => $setting['priority'],
						'choices'     => $choices,
				...
				if ( $style ):
					...
					$value[ "{$key}-style" ] = array(
						'label'       =>  __( 'Font Style', 'dispatch-premium' ),
						'section'     => $setting['section'],
						'type'        => 'select',
						'priority'    => $setting['priority'],
						'choices'     => $choices,
						...
						'description' => array(
							'type' => 'blue',
							'text' => sprintf( __( 'NOTE: Not all fonts support all styles/variants. For example, "Playball" font does not support Bold/Italic/Light variant.<hr />For more info, check the %sGoogle Fonts Library%s', 'dispatch-premium' ), '<a href="https://www.google.com/fonts">', '</a>'),
						),
					);
				...
				if ( $face ):
					...
					$value[ "{$key}-face" ] = array(
						'label'       =>  __( 'Font Face', 'dispatch-premium' ),
						'section'     => $setting['section'],
						'type'        => 'fontface',
						'priority'    => $setting['priority'],
...
add_filter( 'hoot_customizer_prepare_settings', 'hoot_customizer_prepare_typography_settings', 10, 4 );
 ...
 * @return string
 */
function hoot_customizer_fontface_sanitization_function( $name, $type, $setting ) {
	if ( $type == 'fontface' )
		$name = 'hoot_customizer_sanitize_fontface';
	return $name;
}
add_filter( 'hoot_customizer_sanitization_function', 'hoot_customizer_fontface_sanitization_function', 5, 3 );
 ...
 */
function hoot_customizer_sanitize_fontface( $value, $setting ) {
	$choices = hoot_enum_font_faces();
