...
/**
 * Hoot CPT class. This wraps everything up nicely.
 *
 * @since 1.1.0
 */
final class Hoot_CPT {

	/**
	 * The one instance of Hoot_CPT.
	 *
	 * @since 2.0.0
	 * @access private
	 * @var Hoot_CPT The one instance for the singleton.
	 */
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			if ( hoot_theme_supports( 'post-thumbnails', 'post' ) ) {
				add_filter( 'manage_posts_columns', array( $this, 'add_custom_columns' ) );
				add_action( 'manage_posts_custom_column', array( $this, 'custom_columns' ), 10, 2 );
			}
			if ( hoot_theme_supports( 'post-thumbnails', 'page' ) ) {
	...
	function register_post_type() {
		...
		foreach ( $this->options as $post_type => $args ) {
			...
			$singular = ( isset( $labels['singular_name'] ) ) ? $labels['singular_name'] : __( 'Item', 'dispatch-premium' );
			$plural = ( isset( $labels['name'] ) ) ? $labels['name'] : __( 'Item', 'dispatch-premium' );
			$args['labels'] = wp_parse_args( $labels, array(
				...
				'add_new' 			=> sprintf( __( 'Add New %s', 'dispatch-premium' ), $singular ),
				'add_new_item' 		=> sprintf( __( 'Add New %s', 'dispatch-premium' ), $singular ),
				'edit_item' 		=> sprintf( __( 'Edit %s', 'dispatch-premium' ), $singular ),
				'new_item' 			=> sprintf( __( 'New %s', 'dispatch-premium' ), $singular ),
				'view_item' 		=> sprintf( __( 'View %s', 'dispatch-premium' ), $singular ),
				'search_items' 		=> sprintf( __( 'Search %s', 'dispatch-premium' ), $plural ),
				'not_found' 		=> sprintf( __( 'No %s found', 'dispatch-premium' ), $singular ),
				'not_found_in_trash'=> sprintf( __( 'No %s found in Trash', 'dispatch-premium' ), $singular ),
	...
	function add_custom_columns( $custcolumn ) {
		...
		if ( $post->post_type == 'post' || $post->post_type == 'page' )
			$custcolumn['hoot_image_preview'] = __( 'Image Preview', 'dispatch-premium' );
	...
	public function add_options( $options = array() ) {
		$options = apply_filters( 'hoot_theme_cpt' , $options );
	...
	/**
	 * Instantiate or return the one Hoot_CPT instance.
	 *
	 * @since 2.0.0
	 * @access public
	 * @return object
...
/* Initialize class */
global $hoot_cpt;
$hoot_cpt = Hoot_CPT::get_instance();

/* Hook into this action to add options */
do_action( 'hoot_cpt_loaded', $hoot_cpt );