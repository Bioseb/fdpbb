			...
			case 'html':
				...
				if ( $hoot_theme->sliderSettings['html_content_type'] == 'excerpt') {
					if ( empty( $hoot_theme->sliderSettings['excerptlength'] ) )
						...
						$slider[$key]['content'] .= hoot_get_excerpt( $hoot_theme->sliderSettings['excerptlength'] );
				...
				$slider[$key]['button'] = ( empty( $read_more ) ) ? sprintf( __( 'Read More %s', 'dispatch-premium' ), '&rarr;' ) : $read_more;
				break;
