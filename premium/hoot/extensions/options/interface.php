	...
	static function hootoptions_field( $prefix, $group_name='', $field, $val, $echo=true ) {
		...
		case 'typography':
			...
			$typography_options = array(
				'size' => hoot_enum_font_sizes(),
				'face' => hoot_enum_font_faces(),
				'style' => hoot_enum_font_styles(),
				'color' => true
			);
			...
			if ( isset( $field['options'] ) ) {
				...
				$explain_value .= ' ' . __( 'Some Fonts do not support all styles (bold, lighter, italic) For such fonts "Normal" style will be used.', 'dispatch-premium' );
			}
		...
		case 'background':
			...
			// Background Type
			$background_types = hoot_enum_background_type();
			...
			// Background Patterns
			$patterns = hoot_enum_background_pattern();
			...
			if ( $background_options['repeat'] ) :
				...
				$repeats = hoot_enum_background_repeat();
			...
			if ( $background_options['position'] ) :
				...
				$positions = hoot_enum_background_position();
			...
			if ( $background_options['attachment'] ) :
				...
				$attachments = hoot_enum_background_attachment();
		...
		// Icons
		case "icon":
			$iconvalue = hoot_sanitize_fa( $val );
			...
			$output .= '<div id="' . esc_attr( $field_id_prefix . $field['id'] . '-icon-picked' ) . '" class="hoot-of-icon-picked"><i class="' . esc_attr( $iconvalue ) . '"></i><span>' . __( 'Select Icon', 'dispatch-premium' ) . '</span></div>';

			$output .= '<div id="' . esc_attr( $field_id_prefix . $field['id'] . '-icon-picker-box' ) . '" class="hoot-of-icon-picker-box">';

				$output .= '<div class="hoot-of-icon-picker-list"><i class="fas fa-ban hoot-of-icon-none" data-value="0" data-category=""><span>' . __( 'Remove Icon', 'dispatch-premium' ) . '</span></i></div>';
		...
		case "group":
			...
			$settings_defaults = array(
				'add_button' => __( 'Add Another Entry', 'dispatch-premium' ),
				'remove_button' => __( 'Remove Entry', 'dispatch-premium' ),
				'repeatable' => false,
				'sortable' => false,
				'toggleview' => true );
			if ( !empty( $field['settings']['sortable'] ) ) // check if sorting is set and true
				$settings_defaults['title'] = __( 'Group', 'dispatch-premium' );
