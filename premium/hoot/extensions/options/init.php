...
if ( !class_exists( 'Hoot_Options' ) ) :
	...
	class Hoot_Options {
		...
		public function __construct() {
			...
			/* Load functions */
			require_once trailingslashit( HOOT_PREMIUM_EXTENSIONS ) . 'options/includes.php';
		...
		function enqueue( $hook = '' ) {
			...
			$script_uri = hoot_locate_script( trailingslashit( HOOT_JS ) . 'hootoptions' );
			wp_enqueue_script( 'hoot-options-script', $script_uri, array( 'jquery', 'wp-color-picker' ), HOOT_VERSION );
			...
			if ( $load ) {
				...
				$script_uri = hoot_locate_script( trailingslashit( HOOT_JS ) . 'hootoptions-media-uploader' );
				wp_enqueue_script( 'hoot-options-media-uploader', $script_uri, array( 'jquery' ), HOOT_VERSION );

				/* Send data (translation strings) */
				wp_localize_script( 'hoot-options-media-uploader', 'hootoptions_l10n', array(
					'upload' => __( 'Upload', 'dispatch-premium' ),
					'remove' => __( 'Remove', 'dispatch-premium' )
				) );
			...
			$style_uri = hoot_locate_style( trailingslashit( HOOT_CSS ) . 'hootoptions' );
			wp_enqueue_style( 'hootoptions', $style_uri, false,  HOOT_VERSION );
		...
		static function build( $options = array(), $settings = array(), $prefix = '' ) {
			...
			if ( !class_exists( 'Hoot_Options_Interface' ) )
				require_once trailingslashit( HOOT_PREMIUM_EXTENSIONS ) . 'options/interface.php';
		...
		static function validate_option( $input, $option ) {
			...
			if ( has_filter( 'hoot_sanitize_' . $option['type'] ) ) {
				$clean[$id] = apply_filters( 'hoot_sanitize_' . $option['type'], $input[$id], $option );
				return $clean;
			}
		...
		static function uploader( $_id, $_value, $_desc = '', $_name = '' ) {
			...
			$output .= '<input id="' . esc_attr( $id ) . '" class="upload' . $class . '" type="text" name="' . esc_attr( $name ) . '" value="' . $value . '" placeholder="' . __('No file chosen', 'dispatch-premium') .'" />' . "\n";
			if ( function_exists( 'wp_enqueue_media' ) ) {
				if ( ( $value == '' ) ) {
					$output .= '<input id="upload-' . esc_attr( $id ) . '" class="upload-button button" type="button" value="' . __( 'Upload', 'dispatch-premium' ) . '" />' . "\n";
				} else {
					$output .= '<input id="remove-' . esc_attr( $id ) . '" class="remove-file button" type="button" value="' . __( 'Remove', 'dispatch-premium' ) . '" />' . "\n";
				}
			} else {
				$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'dispatch-premium' ) . '</i></p>';
			}
			...
			if ( $value != '' ) {
				...
				if ( $image ) {
					...
					$title = __( 'View File', 'dispatch-premium' );
