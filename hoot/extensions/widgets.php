...
abstract class Hoot_WP_Widget extends WP_Widget {
		...
		?><div class="hoot-widget-field hoot-widget-field-type-<?php echo ( strlen( $type ) < 15 ) ? sanitize_html_class( $type ) : 'custom' ?> hoot-widget-field-<?php echo sanitize_html_class( $id ) ?>"><?php
			...
			switch( $type ) {
				...
				case 'icon':
					...
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picked' ?>" class="hoot-of-icon-picked"><i class="<?php echo esc_attr( $iconvalue ) ?>"></i><span><?php _e( 'Select Icon', 'dispatch-premium' ) ?></span></div>
					<div class="clear"></div>
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picker-box' ?>" class="hoot-of-icon-picker-box">
						<div class="hoot-of-icon-picker-list"><i class="fas fa-ban hoot-of-icon-none" data-value="0" data-category=""><span><?php _e( 'Remove Icon', 'dispatch-premium' ) ?></span></i></div>
						<?php
				...
				case 'image':
					...
					<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-image-selected' ?>" class="hoot-of-image-selected" data-title="<?php _e( 'Select Image', 'dispatch-premium' ) ?>" data-update="<?php _e( 'Set Image', 'dispatch-premium' ) ?>" data-library="image"><span class="hoot-of-image-selected-img" <?php
						if ( !empty( $value ) ) {
							$post = get_post( $value );
						...
						?>></span><span class="hoot-of-image-selected-label"><?php _e( 'Add Image', 'dispatch-premium' ) ?></span></div>
						<a href="#" class="hoot-of-image-remove"><?php _e( 'Remove Image', 'dispatch-premium' ) ?></a>
					<?php
					break;
				...
				case 'group':
					...
					?><div class="hoot-widget-field-group" data-id="<?php echo esc_attr( $id ) ?>">
						...
						?>
						<div id="add-<?php echo rand(1000, 9999); ?>" class="hoot-widget-field-group-add" data-iterator="<?php echo is_array( $value ) ? max( array_keys( $value ) ) : 0; ?>"><?php _e('Add', 'dispatch-premium') ?></div>
					</div>
					<?php
					break;
				...
				case 'collapse':
					...
					?><div class="hoot-widget-field-collapse" data-id="<?php echo esc_attr( $id ) ?>">
						<div class="hoot-collapse-head"><h3><?php if ( !empty( $name ) ) echo esc_html($name); else _e('Group', 'dispatch-premium') ?> <i class="fas fa-sort"></i></h3></div>
	...
	static function get_pages( $number, $post_type = 'page' ){
		...
		// $options_pages[''] = __( 'Select a page:', 'dispatch-premium' );
		$options_pages = array();
		$number = intval( $number );
