...
function hoot_get_wp_link() {
	return sprintf( '<a class="wp-link" href="http://wordpress.org" title="%s">%s</a>', esc_attr__( 'State-of-the-art semantic personal publishing platform', 'dispatch-premium' ), __( 'WordPress', 'dispatch-premium' ) );
}
...
function hoot_get_wp_theme_link( $link = '', $anchor = '' ) {
	...
	$anchor = ( empty( $anchor ) ) ? sprintf( __( '%s WordPress Theme', 'dispatch-premium' ), $name ) : $anchor;

	$title = sprintf( __( '%s WordPress Theme', 'dispatch-premium' ), $name );
...
function hoot_get_loop_title() {
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$loop_title_suffix = ' <span class="loop-title-suffix">' . sprintf( __( '(Page %s)', 'dispatch-premium' ), $paged ) . '</span>';
...
function hoot_get_loop_description() {
	...
	elseif ( is_search() ) {
		...
		$loop_desc = sprintf( __( 'You are browsing the search results for &#8220;%s&#8221;', 'dispatch-premium' ), $search_query );
	...
	elseif ( is_time() )
		$loop_desc = __( 'You are browsing the site archives by time.', 'dispatch-premium' );

	elseif ( is_day() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'dispatch-premium' ), hoot_single_day_title( '', false ) );

	elseif ( is_month() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'dispatch-premium' ), single_month_title( ' ', false ) );

	elseif ( is_year() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'dispatch-premium' ), hoot_single_year_title( '', false ) );

	elseif ( is_archive() )
		$loop_desc = __( 'You are browsing the site archives.', 'dispatch-premium' );
...
function hoot_single_archive_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . __( 'Archives', 'dispatch-premium' );
...
function hoot_single_year_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_date( _x( 'Y', 'yearly archives date format', 'dispatch-premium' ) );
...
function hoot_single_week_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Week %1$s of %2$s', 'dispatch-premium' ), get_the_time( _x( 'W', 'weekly archives date format', 'dispatch-premium' ) ), get_the_time( _x( 'Y', 'yearly archives date format', 'dispatch-premium' ) ) );
...
function hoot_single_day_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_date( _x( 'F j, Y', 'daily archives date format', 'dispatch-premium' ) );
...
function hoot_single_hour_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_time( _x( 'g a', 'hour archives time format', 'dispatch-premium' ) );
...
function hoot_single_minute_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Minute %s', 'dispatch-premium' ), get_the_time( _x( 'i', 'minute archives time format', 'dispatch-premium' ) ) );
...
function hoot_single_minute_hour_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'dispatch-premium' ) );
...
function hoot_search_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Search results for &#8220;%s&#8221;', 'dispatch-premium' ), $search_query );
...
function hoot_404_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . __( '404 Not Found', 'dispatch-premium' );
...
function hoot_developer_data( $commented = true ) {
	ob_start();
	echo get_num_queries() . ' ' . __( 'queries.', 'dispatch-premium' ) . ' ';
	timer_stop(1);
	echo ' ' . __( 'seconds.', 'dispatch-premium' ) . ' ' . ( memory_get_peak_usage(1) / 1024 ) / 1024 . 'MB';
	$out = ob_get_contents();
	ob_end_clean();
	return $out;
}